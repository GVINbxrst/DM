...
try {
    ...
    celery -A src.worker.config:celery_app worker --beat --loglevel=$LogLevel --concurrency=$Concurrency
    } elseif ($Flower) {
    ...
    Start-Process -NoNewWindow -FilePath "celery" -ArgumentList "-A", "src.worker.config:celery_app", "worker", "--loglevel=$LogLevel", "--concurrency=$Concurrency"

        # Запускаем Flower
    celery -A src.worker.config:celery_app flower
    } elseif ($Queue) {
        Write-Host "📋 Обработка очереди: $Queue" -ForegroundColor Cyan
    celery -A src.worker.config:celery_app worker --loglevel=$LogLevel --concurrency=$Concurrency --queues=$Queue
    } else {
    celery -A src.worker.config:celery_app worker --loglevel=$LogLevel --concurrency=$Concurrency
    }
} catch {
