...
init_database() {
    if command -v docker &> /dev/null && command -v docker-compose &> /dev/null; then
        ...
        if docker-compose ps postgres | grep -q "Up"; then
            # Применяем миграции Alembic (универсально для прод/дев)
            if python3 -m alembic upgrade head; then
                print_success "База данных инициализирована (alembic upgrade head)"
            else
                print_warning "Не удалось применить миграции Alembic"
            fi
        else
