# DiagMod API Server - PowerShell Launch Script
# Запуск FastAPI сервера в режиме разработки

param(
    [string]$BindHost = "0.0.0.0",
    [int]$Port = 8000,
    [switch]$Reload = $true,
    [switch]$Help
)

if ($Help) {
    Write-Host "DiagMod API Server" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Параметры:"
    Write-Host "  -BindHost <ip> IP адрес для привязки (по умолчанию: 0.0.0.0)"
    Write-Host "  -Port <port>   Порт для прослушивания (по умолчанию: 8000)"
    Write-Host "  -Reload        Автоперезагрузка при изменениях (по умолчанию: включена)"
    Write-Host "  -Help          Показать эту справку"
    Write-Host ""
    Write-Host "Примеры:"
    Write-Host "  .\run-api.ps1                    # Стандартный запуск"
    Write-Host "  .\run-api.ps1 -Port 8080         # Запуск на порту 8080"
    Write-Host "  .\run-api.ps1 -BindHost 127.0.0.1 # Только локальный доступ"
    exit 0
}

Write-Host "🚀 Запуск DiagMod API Server..." -ForegroundColor Green
Write-Host "📡 Сервер будет доступен по адресу: http://$BindHost`:$Port" -ForegroundColor Yellow
Write-Host "📚 API документация: http://$BindHost`:$Port/docs" -ForegroundColor Yellow
Write-Host "📊 Метрики: http://$BindHost`:$Port/metrics" -ForegroundColor Yellow
Write-Host ""

# Проверяем активацию виртуального окружения
if (-not $env:VIRTUAL_ENV) {
    Write-Warning "Виртуальное окружение не активировано. Активируем автоматически..."
    if (Test-Path ".venv\Scripts\Activate.ps1") {
        & ".venv\Scripts\Activate.ps1"
        Write-Host "✅ Активировано .venv" -ForegroundColor Green
    } elseif (Test-Path "venv\Scripts\Activate.ps1") {
        & "venv\Scripts\Activate.ps1"
        Write-Host "✅ Активировано venv" -ForegroundColor Green
    } else {
        Write-Error "Виртуальное окружение не найдено. Создайте .venv: python -m venv .venv"
        exit 1
    }
}

# Гарантируем, что корень проекта в PYTHONPATH для импорта пакета src
$env:PYTHONPATH = (Resolve-Path ".").Path

# Запускаем API сервер
try {
    if ($Reload) {
    uvicorn src.api.main:app --reload --host $BindHost --port $Port
    } else {
    uvicorn src.api.main:app --host $BindHost --port $Port
    }
} catch {
    Write-Error "Ошибка запуска API сервера: $_"
    Write-Host "Убедитесь, что:"
    Write-Host "  1. Установлены все зависимости (.\setup.ps1 -Dev)"
    Write-Host "  2. Запущена база данных (docker-compose up -d postgres redis)"
    exit 1
}
