...
async def upload_csv_file(
    ...
    # Fallback: создаём минимальное оборудование при отсутствии
        ...
        if 'Не удалось определить оборудование' in str(e) or isinstance(e, CSVLoaderError):
            try:
                ...
                os.unlink(tmp_path)
                # В dev-режиме возвращаем деталь ошибки для ускорения отладки
                _st = get_settings()
                detail_msg = (
                    f"Ошибка обработки файла: {inner}" if _st.is_development else "Ошибка обработки файла"
                )
            ...
            _st = get_settings()
            detail_msg = (
                f"Ошибка обработки файла: {e}" if _st.is_development else "Ошибка обработки файла"
            )
            raise HTTPException(status_code=500, detail=detail_msg)
    finally:
    ...
    # Постановка Celery-задачи (передаём только raw_id — данные уже в БД)
    try:
        ...
        st = get_settings()
        # Локальный быстрый путь: без брокера, выполняем асинхронно
        import os as _os
        if st.is_testing or _os.getenv('CELERY_TASK_ALWAYS_EAGER', '0').lower() in {'1','true','yes'}:
        ...
        _st = get_settings()
        detail_msg = (
            f"Ошибка постановки задачи Celery: {e}" if _st.is_development else "Ошибка постановки задачи Celery"
        )
        raise HTTPException(status_code=500, detail=detail_msg)
    finally:
