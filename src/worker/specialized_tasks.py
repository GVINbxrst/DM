...
from src.database.models import RawSignal, Feature, Equipment, ProcessingStatus, Prediction, Forecast
        ...
        for eq in equipment_list:
            ...
            reports.append({
                ...
                'last_forecast': {
                    ...
                    'max_anomaly_probability': (float(last_forecast.probability_over_threshold) if last_forecast and last_forecast.probability_over_threshold is not None else None),
                    'recommendation': (last_forecast.forecast_data.get('summary', {}).get('recommendation') if last_forecast and last_forecast.forecast_data else None)
                }
            })
        total = len(reports)
