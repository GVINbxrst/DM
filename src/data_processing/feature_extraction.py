...
class FeatureExtractor:
    ...
    async def _save_features_to_db(
        ...
        # Идемпотентность: если такой признак уже есть (по raw_id+window_start) — не дублируем
        from sqlalchemy import select  # локальный импорт, чтобы не тащить наверх
        from src.database.models import Feature as FeatureModel  # избегаем путаницы с локальной переменной
        existing = await session.execute(
            select(FeatureModel.id).where(
                FeatureModel.raw_id == feature_record.raw_id,
                FeatureModel.window_start == feature_record.window_start,
            )
        )
        existing_id = existing.scalar_one_or_none()
        if existing_id:
            # Уже существует — используем существующий id и не вставляем дубль
            feature_record.id = existing_id
        else:
                ...
                if raw_signal:
                    ...
                    if vals:
                        ...
                        if row:
                            ...
                            if prev_samples:
                                prev_mean = float(row.rms_mean) if row.rms_mean is not None else 0.0
                                row.rms_mean = float((prev_mean * prev_samples + rms_mean) / (prev_samples + 1))
                            else:
                                row.rms_mean = float(rms_mean)
                            row.rms_max = max(float(row.rms_max) if row.rms_max is not None else rms_mean, rms_mean)
                            row.rms_min = min(float(row.rms_min) if row.rms_min is not None else rms_mean, rms_mean)
                            ...
                            session.add(HourlyFeatureSummary(
                                ...
                                rms_max=float(rms_mean),
                                rms_min=float(rms_mean),
                                samples=1
                            ))
