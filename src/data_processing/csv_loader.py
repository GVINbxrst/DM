...
class CSVLoader:
    ...
    async def _process_csv_file_batches(
        ...
        with open(file_path, 'r', encoding='utf-8') as csvfile:
            ...
            corrupt_logged = 0
            batch_index = 0  # индекс пачки в рамках файла
            ...
            for row in reader:
                ...
                if batch_size >= self.batch_size:
                    batch_t0 = time.time()
                    # file_hash указываем только для первой пачки, чтобы избежать
                    # нарушения уникального индекса uq_raw_signals_file_hash
                    current_fhash = file_hash if batch_index == 0 else None
                    save_result = await self._save_batch_to_db(
                        batch_data, equipment_id, sample_rate,
                        recorded_at, current_fhash, file_path.name, session,
                        {**(metadata or {}), 'batch_index': batch_index}
                    )
